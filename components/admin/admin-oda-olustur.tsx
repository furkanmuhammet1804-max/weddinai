...
import { ArrowLeft, Heart, Lock, Loader2, Eye, EyeOff, Sparkles } from "lucide-react";
...
export function AdminOdaOlustur() {
  ...
  async function gonder(e: React.FormEvent) {
    ...
    if (form.kullaniciAdi.trim().length < 3) {
      setHata("Kullanıcı adı (oda kodu) en az 3 karakter olmalı.");
      return;
    }
    ...
    <div className="mx-auto max-w-xl">
      ...
      >
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Müşterin için gizli, şifreli bir etkinlik odası oluştur. Oda 7 gün
          sonra otomatik silinir (süreyi sonra uzatabilirsin).
        </p>
        ...
        <form onSubmit={gonder} className="mt-6 space-y-4">
          ...
          <div>
            <label className="text-sm font-medium">Kullanıcı Adı (Oda Kodu)</label>
            <p className="mt-0.5 text-xs text-muted-foreground">
              Müşteri giriş ekranında bunu yazacak. Şifreyle birlikte onunla
              paylaşacaksın.
            </p>
            <input
              value={form.kullaniciAdi}
              onChange={(e) =>
                guncelle("kullaniciAdi", e.target.value.replace(/\s+/g, ""))
              }
              placeholder="örn. furkanbengisu"
              autoCapitalize="off"
              autoCorrect="off"
              spellCheck={false}
